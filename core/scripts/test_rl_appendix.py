...
def load_model(model_name, raise_not_found=True, device='cuda:0'):
...
def do_experiments(args, log_dir, model_dir, ignore_rules):
    ...
    if use_pretrained:
        prev_args = torch.load(model_dir + f'/{continue_pretrained}/args.pkl', map_location=torch.device(device))
        for a in vars(args):
    ...
    if args.pretrained_model:
        model = load_model(args.pretrained_model, raise_not_found=True, device=device)
    ...
    for i in range(len(ignore_rules) + 1):
        ...
        obs = torch.cat([j.image for j in logs['observations_per_episode']], dim=0)
        missions = [j.instr for j in logs['observations_per_episode']]
        rewards = torch.cat([torch.FloatTensor([i]) for j in logs['rewards'] for i in j], dim=0)
...
def main():
    ...
    # return
    file = open(os.path.join(root, 'run/logs/run_paths.csv'))
    ...
    for data in rows:
        if (data[2] == 'NPS-D-MFC-ConcatAC' or data[2] == 'NPS-D-MFC-ConcatMem' or data[2] == 'NPS-IC-GRU' or data[2] == 'NPS-D-instr-concat') and \
                (data[4] == 'BabyAI-ActionObjDoor-v0'):
    ...
    for key in servers.keys():
        if key == '18':
            ...
            model_dir = os.path.join(root, 'run/logs_112/logs')
        else:
