...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.rule_network = RuleNetwork(args.slot_dim, 4, num_rules = args.num_rules, rule_dim = args.rule_dim, 
            query_dim = 32, value_dim = 64, key_dim = 32, num_heads = 4, 
            dropout = 0.1, design_config = self.design_config)
...
class MNISTModel(nn.Module):
    ...
    def __init__(self, args):
        ...
        if self.num_rules > 0:
            ...
            self.rule_network = RuleNetwork(self.output_size, 2, num_rules = args.num_rules, rule_dim = args.rule_dim, query_dim = 32, 
                value_dim = 64, key_dim = 32, num_heads = 4, dropout = 0.1, design_config = self.design_config)
    ...
    def forward(self, x, h_prev, inp_transforms, rule_mask = None):
        ...
        #print(encoded_input.size())
        if self.num_rules > 0:
            print(encoded_input.shape, 'rule net')
